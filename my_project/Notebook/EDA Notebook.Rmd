...
```{r}
# Load all 4 datasets 
setwd("/Users/hk_cheng/Downloads/my_projectt/my_project")
data_path <- "data"

draft_history <- read_csv(file.path(data_path, "draft_history.csv"))
nba_2020 <- read_csv(file.path(data_path, "nba_2020.csv"))
nba_2021 <- read_csv(file.path(data_path, "nba_2021.csv"))
nba_2022 <- read_csv(file.path(data_path, "nba_2022.csv"))
```
...
We can now conduct visualizations of the distribution of average points earned per game for both NBA players and rookie players in both 2020 and 2022, and we can also present the mean value to achieve the following objectives (Note: the year 2021 was excluded because our goal was to present a trajectory over the rookie players’ three-year development)
...
Just like how we visualized the distribution of average points earned per game, we can also explore the distribution of rebounds per game and presented the mean value, and investigate a pattern just like observing in the distribution of average points earned per game.
...
Let us see if there are any difference in terms of assists.
...
To visualize and analyze data in between round 1 and round 2, we not only need to filter out the stats of rookies but also have to store them separately according  to their draft rounds.
...
In order to display rookies' performances categoraized by their positions and see their performance trend in 3 continous years, we need to first modify our data frames again. We want to first further split the data frames according to the positions of rookie players, then merge data frames of different seasons together so that we are able to visualize the trends of rookie's performance in 3 years.
...
#### First Round vs Second Round Point Guards
...
levels1 <- c("Anthony Edwards","James Wiseman","LaMelo Ball","Patrick Williams","Isaac Okoro","Onyeka Okongwu","Killian Hayes","Obi Toppin","Deni Avdija","Jalen Smith","Devin Vassell","Tyrese Haliburton","Kira Lewis Jr.","Aaron Nesmith","Cole Anthony","Isaiah Stewart","Aleksej Pokusevski","Josh Green","Saddiq Bey","Precious Achiuwa","Tyrese Maxey","Zeke Nnaji","Leandro Bolmaro","R.J. Hampton","Immanuel Quickley","Payton Pritchard","Udoka Azubuike","Jaden McDaniels","Malachi Flynn","Desmond Bane")

plot_1_pg <- ggplot(merged_1_pg, aes(fill = year, x = factor(Player, levels = levels1), y = AST)) + 
...
levels2 <- c("Tyrell Terry", "Vernon Carey Jr.", "Daniel Oturu", "Theo Maledon", "Xavier Tillman", "Tyler Bey", "Vit Krejci", "Saben Lee", "Elijah Hughes", "Robert Woodard II", "Tre Jones", "Nick Richards", "Jahmi'us Ramsey", "Marko Simonovic", "Jordan Nwora", "CJ Elleby", "Yam Madar", "Nico Mannion", "Isaiah Joe", "Skylar Mays", "Justinian Jessup", "Kenyon Martin Jr.", "Cassius Winston", "Cassius Stanley", "Jay Scrubb", "Grant Riller", "Reggie Perry", "Paul Reed", "Jalen Harris", "Sam Merrill")

plot_2_pg <- ggplot(merged_2_pg, aes(fill = year, x = factor(Player, levels = levels2), y = AST)) + 
  geom_bar(position = "dodge", stat = "identity", width = 0.5) + 
  scale_fill_viridis_d() + 
...
#### First Round vs Second Round Centers

```{r}
# Create bar chart for first round rookies who registered as Centers
# Creating levels allows x-axis to present player names in the order of draft picks (from left to right)
plot_1_c <- ggplot(merged_1_c, aes(fill = year, x = factor(Player, levels = levels1), y = TRB)) + 
  geom_bar(position = "dodge", stat = "identity", width = 0.5) + 
  scale_fill_viridis_d() + 
...
# Creating levels allows x-axis to present player names in the order of draft picks (from left to right)
plot_2_c <- ggplot(merged_2_c, aes(fill = year, x = factor(Player, levels = levels2), y = TRB)) + 
  geom_bar(position = "dodge", stat = "identity", width = 0.5) + 
  scale_fill_viridis_d() + 
...
#### First Round vs Second Round Shooting Guards

```{r}
# Create bar chart for first round rookies who registered as shooting guards
# Creating levels allows x-axis to present player names in the order of draft picks (from left to right)
plot_1_sg <- ggplot(merged_1_sg, aes(fill = year, x = factor(Player, levels = levels1), y = PTS)) + 
  geom_bar(position = "dodge", stat = "identity", width = 0.5) + 
  scale_fill_viridis_d() + 
...
# Creating levels allows x-axis to present player names in the order of draft picks (from left to right)
plot_2_sg <- ggplot(merged_2_sg, aes(fill = year, x = factor(Player, levels = levels2), y = PTS)) + 
  geom_bar(position = "dodge", stat = "identity", width = 0.5) + 
  scale_fill_viridis_d() + 
